...
JSONL_PATH = PROJECT_ROOT / "datasets" / "microsoft-AgentRx" / "tau_retail.jsonl"
...
CATEGORY_MAP = {
    ...
    "Underspecified User Intent": "intent_not_supported",
...
def process_trajectories() -> tuple[dict[str, list[ErrorRecord]], int]:
    ...
    with open(JSONL_PATH) as f:
        for traj_idx, line in enumerate(f):
            d = json.loads(line)
            traj_id = d.get("trajectory_id") or f"tr_{traj_idx}"
            ...
            for fail in d.get("failures", []):
                ...
                record = ErrorRecord(
                    error_id=f"agentrx_tr_{traj_id}_{failure_id}",
                    dataset="agentRx",
                    error_type=cat,
                    error_subtype=None,
                    is_dedup=False,
