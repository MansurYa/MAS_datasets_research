...
100k символов вероятность успеха падает с ~85% до ~3%. Жёсткой стенки на
конкретном номере шага нет. Это — прямая реализация Категории 2 в данных.
...
- Данные: `work/data/`
- Документация: `work/docs/` (knowledge base, концепции)
- **Текущая задача: читать `work/specs/TZ_3.md`**
...
work/               ← ТЕКУЩАЯ РАБОТА
  specs/            ← TZ_1.md, TZ_2.md, TZ_3.md …
  scripts/          ← новые скрипты
  reports/          ← TZ_1_report.md, TZ_2_v2_report.md, TZ_3_baseline_eda_report.md …
  data/             ← выходные CSV и графики
  docs/             ← концепции, knowledge base (invalid_invocation, baseline_trajectory_physics)
...
6. nebius: длина контекста считается в **символах** (не токенах), tiktok не установлен.
   При параметризации момента ошибки — использовать `n_chars`, не `step_idx`.
   Жёсткая стенка контекста: ~100 000 символов. Подробности в `work/docs/baseline_trajectory_physics.md`.
